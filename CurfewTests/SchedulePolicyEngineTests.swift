...
struct SchedulePolicyEngineTests {
    ...
    func weakeningChangeHas24HourCooldown() {
        ...
        #expect(effectiveDate.timeIntervalSince(requestedAt) == 86_400)
    }
    ...
    func stricterChangeAppliesNextDay() {
        ...
        let components = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: effectiveDate)
...
struct ScheduleResolutionTests {
    ...
    func scheduleWindowResolvesSpringForwardGap() throws {
        ...
        let reference = try #require(calendar.date(from: .init(year: 2026, month: 3, day: 8, hour: 12, minute: 0)))
    ...
    func scheduleSummarySentenceForTomorrow() throws {
        ...
        let reference = try #require(calendar.date(from: .init(year: 2026, month: 2, day: 2, hour: 9, minute: 0)))
        ...
        #expect(normalizedSentence == "Tomorrow, your computer locks at 6:00 PM and unlocks at 8:00 AM.")
    }
}
