...
struct WarningNotificationPayload: Equatable, Sendable {
    let title: String
    let body: String
...
struct WarningNotificationCategoryDefinition: Equatable, Sendable {
...
final class WarningNotificationManager: NSObject {
    ...
    static func payload(for stage: WarningStage) -> WarningNotificationPayload? {
        switch stage {
        case .thirtyMinutes:
            return WarningNotificationPayload(
                title: "Curfew",
    ...
    private func registerCategories() {
        let categories = Self.categoryDefinitions().map { definition in
            let actions = definition.actionIdentifiers.compactMap { identifier -> UNNotificationAction? in
