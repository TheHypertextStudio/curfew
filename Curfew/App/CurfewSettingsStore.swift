...
struct OverrideEvent: Codable, Equatable, Sendable {
    var timestamp: Date
...
struct PendingScheduleChange: Codable, Equatable, Sendable {
...
struct CurfewSettings: Codable, Equatable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        schedule = try container.decode(WeeklySchedule.self, forKey: .schedule)
        pendingScheduleChange = try container.decodeIfPresent(PendingScheduleChange.self, forKey: .pendingScheduleChange)
        hasCompletedInitialSetup = try container.decodeIfPresent(Bool.self, forKey: .hasCompletedInitialSetup) ?? false
        extensionWeeklyLimit = try container.decode(Int.self, forKey: .extensionWeeklyLimit)
        extensionDurationMinutes = try container.decode(Int.self, forKey: .extensionDurationMinutes)
        overrideWeeklyLimit = try container.decode(Int.self, forKey: .overrideWeeklyLimit)
        overrideDurationMinutes = try container.decode(Int.self, forKey: .overrideDurationMinutes)
        resetWeekday = try container.decode(Weekday.self, forKey: .resetWeekday)
        autoShutdownEnabled = try container.decode(Bool.self, forKey: .autoShutdownEnabled)
        autoShutdownDelayMinutes = try container.decode(Int.self, forKey: .autoShutdownDelayMinutes)
        warningIntervals = (try container.decodeIfPresent(WarningIntervals.self, forKey: .warningIntervals) ?? .default).normalized
    }
