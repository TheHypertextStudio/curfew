...
- [-] Enforce coordinator-signed remote lock commands in the privileged daemon. The app stages only opaque `0.0.8` envelopes; the installed daemon wakes every 15 seconds, requires an authenticated account/device enrollment, fetches the bounded public JWKS from `curfew-sync.hypertext.studio`, verifies ES256 claims, commits replay/result/deadline state atomically with private file permissions, and feeds the effective deadline into the existing enforcement loop. Local MCP and remote MCP now conform to one daemon-injected backend boundary: synchronization failures are isolated, durable deadlines remain enforceable during transport failure, and the strongest active deadline wins without transport-specific policy in the daemon loop. A durable two-phase exchange returns authenticated delivery acknowledgements and terminal results through the app, retaining them across offline retries until the daemon sees exact coordinator acceptance. When Curfew is running, that authenticated result is projected through the account-sync callback into the same stronger-only durable deadline and immediately presents the lockout surface. The root-owned shadow is the authority while the login session is locked because macOS may data-protect the user's Application Support tree; model tests therefore inject temporary stores and never write synthetic deadlines into live user state. Signed-build installation and live coordinator proof remain incomplete. (v0.2)
